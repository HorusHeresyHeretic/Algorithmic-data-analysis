...
Человек, который её нарисовал, не смог написать алгоритм, в котором `ML code` сам решает, какую модель отдать в прод заранее перебрав несколько опций и сравнив их по значению среднеквадратичной ошибки. Я вам даже больше скажу, размер тестовой выборки тоже можно отдать на откуп компьютеру.
