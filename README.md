...
если посмотрев мой код напишите своё, не пугайтесь когда после запуска снизу вылезет простыня.

Таким образом математика отдельно - данные отдельно. Можно написать большой блок различных операций, который вы хотите применить к объектам внутри датафрейма большой функций, а потом гонять её генератором по всему датафрейму. Со временем алгоритм исследования данных станет выглдяить так:
...
ит.д
 ...
 С другой стороны генератор который мы передали в фукнцию вычисления глупый, остановится как только засбоит одна из операций счисления которая записана в фукнцию. Например в  коде, который я используя для примеров (и который дают по кускам слайдами) у меня было более 50 операций счисления и местами работать с исключениями было трудозатратно.
...
С одной стороны работать с данными по ключу в потоке приятно -  можно сначала написать все интересныеварианты вычислений, а уже потом пытаться применять весь блок к объектам. С другой стороны генератор который мы передали в фукнцию вычисления остановится как только засбоит одна из операций счисления которая записана в фукнцию, например в  коде, который я используя для примеров (и который дают по кускам слайдами) у меня было более 50 операций счисления и местами работать с исключениями было трудозатратно.
