...
В том же потоке запускаем функцию кеширования. Кеш это one = pd.DataFrame, складывать вывод фукнции будем туда.  (да да я знаю, 98% людей которым я показал это, кричали что то про инквизицию). Кеш это one = pd.DataFrame, складывать вывод фукнции будем туда. 
