...
1Ска (помимо того что она пишет даты по госту, а не в стандарте iso) вообщемто не самый приятный источник данных для DS, но другой у нас нет. Именно она хранит практически неисчерпаемый источник raw data, приобщайтесь.
...
На слайде типичный вывод из 1ски глазами DS. ЧТо удобно - у каждого документа в 1ске всегда есть номер документа, а также как правило дополнительный уникальный id, то к чему этот номер относиться, плюс справочники которые оператор 1с использует для заполнения документа. 

Документы с датами для DS всегда имеют большую ценность, остальное можно смело отдавать в отдел `ананлитики`(тем людям, которые пользуются sql, excel и dash в рамках приложения) они быстрее и лучше дадут результат там, где он есть. Всё остальное на нас.
