...
## ML, DS или DEV? нет - программист исследователь!

Насколько я знаю у Yorko именно это в трудовой книжке записанно. Алгоритм который думает за тебя - лучше чем модель которую ты обучил. Алгоритм может обучить модель, а модель не может заменить алгоритм. Я знаю что гайды, которые я пишу заходят только узкому кругу лиц, всем остальным я предлагаю провесьти эксперимент: прежде чем отбраковывать один из методов из доступного списка по метрикам оценки точности - разбейте каждый метод внутри на 2 простых алгоритмических варианта и сравните две метрики. Например, можно просто предложить машине на выбор разную длинну выборки для обучения. 70-30 и 60-40.
