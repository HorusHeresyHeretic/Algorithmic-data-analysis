...
Для примера я буду использовать датасет полученный из корпоративной 1С, первичный документ учёта ремонта ТС в РММ крупного предприятия. Принято считать, что данные это новая нефть, возможно, но в России цена добычи - золото.  1Ска (помимо того что она пишет даты по госту, а не в стандарте iso) вообщемто не самый приятный источник данных для DS, но другого у нас нет. 

Из плюсов именно она как правило хранит неисчерпаемый источник raw data и подчинятьеся учётной политике. Из минусов даже если учётная политика глобальна, как правило в РФ каждое предприятие обычно ведёт учёт в 1C по разному, до того как не встретит DS. 

Пример алгоритма, который можно написать вокруг данных из 1С можно посмотреть [здесь](https://github.com/HorusHeresyHeretic/Algorithmic-data-analysis/blob/main/i7%2B64GB%2Bruntime%2B3%2Bhours.ipynb). Может ругаться на unable to render - открыть в raw, потом снова открыть нормально, заработает. Поскольку в данных используются временные ряды, фразу `пример для 1С` можно зачеркнуть и прочто считать это примером создания алгоритма анализа данных построеннего вокруг временны рядов. 


На входе 8 датасетов из 8 разных предприятий (пример файла исходника ниже на картинке), на выходе 300+ файлов. Это достигаеться за счёт 
