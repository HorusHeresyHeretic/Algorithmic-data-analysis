...
7. Сам заказчик иногда толком не знает что он хочет получить или ухватится за промежуточный результат и откажеться от первоначальной идеи. Поэтому имеет смысл показывать промежуточные таблицы руководителям (кто захочет - разберётся). Примерно треть вывода в этом алгоритме это то, что увидели люди в промежуточных таблицах и попросили допилить, своей рукой (карандашом на распечатках) выводя логику будуших KPI. Таким образом разработка DS перестаёт быть тёмным ящиком для управленцев ниу и о рсиках разработки ради разработки не стоит забывать.
