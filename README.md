...
Для примера я буду использовать датасет полученный из корпоративной 1С, первичный документ учёта ремонта ТС в РММ крупного предприятия. Принято считать, что данные это новая нефть, возможно, но в России цена добычи - золото. 

1Ска (помимо того что она пишет даты по госту, а не в стандарте iso) вообщемто не самый приятный источник данных для DS, но другого у нас нет. Именно она хранит практически неисчерпаемый источник raw data, приобщайтесь.

Алгоритм целиком можно посмотреть [здесь](https://github.com/HorusHeresyHeretic/Algorithmic-data-analysis/blob/main/i7%2B64GB%2Bruntime%2B3%2Bhours.ipynb). Может ругаться на unable to render - открыть в raw, потом снова открыть нормально, заработает. На входе 8 датасетов из 8 разных предприятий (пример файла исходника ниже на картинке), на выходе 300+ файлов. Это достигаеться за счёт 

На входе 8 датасетов из 8 разных предприятий, данные в которых не были целиком унифицрованны. Даже если учётная политика у вас глобальная, всё равно (и как правило в производственных компания в РФ это так) каждое предприятие обычно ведёт учёт в 1C по разному, до того как не встретит DS.
...
# На принтах и я могу, а как с этим работать? часть 3.
## Будешь великим программистом (а потом захочешь бросить всё это).
...
Поэтому вывод фукнции надо сложить заново в датафрейм, для этого надо использоать кеш, пример: дёргаём алгоритм вокруг Холта-Винтерса на 1К строк который также написан через генератор (возвращает total и roll) и обернут в функцию.
...
# Не лучше ли тогда писать через try? часть 5.
## try лучше использовать для управления алгоритмом.

Последняя и пожалуй самая важная часть. 
