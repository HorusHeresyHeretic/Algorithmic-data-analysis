...
Вы отдадите только модель в pipeline, например как как написано [здесь](https://habr.com/ru/post/591873/). Модель будет работать по метрикам и отклоненям в проде, всё. А когда все возможные методы, дающие приемлемый результат кончаться, вы просто скажите: `данные плохие - это невозможно`.
