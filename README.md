...
Из плюсов именно 1Ска хранит неисчерпаемый источник raw data и подчинятьеся учётной политике. Из минусов даже если учётная политика глобальна, как правило каждое предприятие обычно ведёт учёт `привычным` им образом, до того как встретит DS. 
...
На входе 8 датасетов из 8 разных предприятий (пример файла исходника ниже на картинке), на выходе 300+ файлов. 90% людей которым я показывал это код, кричали что то про инквизицию (концепция dataframe как кеш не для всех), поэтому объяснять я буду слайдами. 

Остальные считают, что мне нужно брать по мульту в месяц с заказчиков, поскольку концепция существенно облегчает работу смежных DBA и вообще это больше похоже на готовое приложение, которые осталось обернуть в web, чем на обычную тетрадку с идеями.
