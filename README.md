...
Конечно, с точки зрения книжки Дрейка выглядит всё это не красиво, Python это компактные читаемые классы, а не нагромаждение фукнций, но! с точки зрения DS `таков путь`. На том же Mlopen учат как писать код в тетрадках, не более. А как вы собиратетесь работать с этими тетрадками? выбросите? - отдадите только модель в pipeline, например как как написано [здесь](https://habr.com/ru/post/591873/).
