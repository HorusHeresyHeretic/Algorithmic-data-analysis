...
Из плюсов именно она как правило хранит неисчерпаемый источник raw data и подчинятьеся учётной политике. Из минусов даже если учётная политика глобальна, как правило в РФ каждое предприятие обычно ведёт учёт в 1C по разному, до того как не встретит DS.  Пример алгоритма, который можно написать вокруг данных из 1С можно посмотреть [здесь](https://github.com/HorusHeresyHeretic/Algorithmic-data-analysis/blob/main/i7%2B64GB%2Bruntime%2B3%2Bhours.ipynb). Может ругаться на unable to render - открыть в raw, потом снова открыть нормально, заработает. 
