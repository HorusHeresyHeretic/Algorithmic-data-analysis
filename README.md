...
и посмотреть что получилось в принтах дёрнув все го лишь одну функцию
...
С одной стороны работать с данными по ключу в потоке приятно -  с другой стороны генератор который мы передали в фукнцию вычисления остановиться как только засбоит одна из операций счисления которую записла в фукнции, например в  коде, который я используя для примеров (и который дают по кускам слайдами) 
